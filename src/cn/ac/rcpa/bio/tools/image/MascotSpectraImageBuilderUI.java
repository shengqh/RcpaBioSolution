...
public class MascotSpectraImageBuilderUI extends AbstractUI {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1028710277475355052L;
	...
	private JRcpaFileField txtMgfFile = new JRcpaFileField("MgfFile", new OpenFileArgument("Mascot Generic Format", new String[] { "mgf", "msm" }), true);
		...
		try {
			...
			try {
				...
				while (mgfi.hasNext()) {
					...
					mgfScanPklMap.put(pkl.getExperimental() + "_" + pkl.getFirstScan(), pkl);
				}
			} finally {
				br.close();
			}
			...
			checkSequenceForAcetylProteinNtermModification(dynamicModification, peptides);
			...
			for (int i = 0; i < peptides.size(); i++) {
				...
				String scan = experimental + "_" + peptide.getFileName().getFirstScan();

				if (!mgfScanPklMap.containsKey(scan)) {
					System.out.println("Cannot find " + peptide.getFileName().getFirstScan() + " of " + experimental + " in " + mgfFilename);
					continue;
				}
				...
				if (!bMatched) {
					throw new AccessException(peptide.getFileName().getLongFilename() + "," + "Precursor m/z are not matched. Check modification setting please.\nReal precursor m/z = "
							+ pkl.getPrecursorMZ() + " ; calculated precursor m/z = " + theoreticalMz);
				}
			...
			JOptionPane.showMessageDialog(this, "Succeed", "Congradulation", JOptionPane.INFORMATION_MESSAGE);
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public static void checkSequenceForAcetylProteinNtermModification(Map<Character, Double> dynamicModification, List<SlimIdentifiedPeptide> peptides) {
		char acetylProteinNtermDynamicModificationChar = ' ';
		for (SlimIdentifiedPeptide pep : peptides) {
			if (pep.getModification() != null && pep.getModification().contains("Acetyl (Protein N-term)")) {
				...
				if (Character.isLetter(matchedSeq.charAt(startIndex))) {
					if (acetylProteinNtermDynamicModificationChar == ' ') {
						...
						for (int i = 1; i < MODIFICATION_CHAR.length(); i++) {
							if (!dynamicModification.containsKey(MODIFICATION_CHAR.charAt(i))) {
								acetylProteinNtermDynamicModificationChar = MODIFICATION_CHAR.charAt(i);
								dynamicModification.put(acetylProteinNtermDynamicModificationChar, 42.010559);
								break;
							}
						}
					}

					String newSeq = matchedSeq.substring(0, startIndex) + acetylProteinNtermDynamicModificationChar + matchedSeq.substring(startIndex);
