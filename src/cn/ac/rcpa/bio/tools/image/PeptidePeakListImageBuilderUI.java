...
public class PeptidePeakListImageBuilderUI extends AbstractUI {
	...
	private JRcpaComponentProxy lblIon = new JRcpaComponentProxy(ion, 0.0,
			GridBagConstraints.EAST, GridBagConstraints.NONE, 1);

	private JRcpaObjectCheckBox<IonType> bIons = new JRcpaObjectCheckBox<IonType>(
			"B_IONS", "B Ions", 1, IonType.B);

	private JRcpaObjectCheckBox<IonType> b2Ions = new JRcpaObjectCheckBox<IonType>(
			"B2_IONS", "B2 Ions", 2, IonType.B2);

	private JRcpaObjectCheckBox<IonType> b_NH3Ions = new JRcpaObjectCheckBox<IonType>(
			"B_NH3IONS", "B-NH3 Ions", 3, IonType.B_NH3);

	private JRcpaObjectCheckBox<IonType> b_H2OIons = new JRcpaObjectCheckBox<IonType>(
			"B_H2OIONS", "B-H2O Ions", 4, IonType.B_H2O);

	private JRcpaObjectCheckBox<IonType> aIons = new JRcpaObjectCheckBox<IonType>(
			"A_IONS", "A Ions", 5, IonType.A);

	private JRcpaObjectCheckBox<IonType> cIons = new JRcpaObjectCheckBox<IonType>(
			"C_IONS", "C Ions", 6, IonType.C);

	private JRcpaHorizontalComponentList bIonList = new JRcpaHorizontalComponentList();

	private JRcpaObjectCheckBox<IonType> yIons = new JRcpaObjectCheckBox<IonType>(
			"Y_IONS", "Y Ions", 1, IonType.Y);

	private JRcpaObjectCheckBox<IonType> y2Ions = new JRcpaObjectCheckBox<IonType>(
			"Y2_IONS", "Y2 Ions", 2, IonType.Y2);

	private JRcpaObjectCheckBox<IonType> y_NH3Ions = new JRcpaObjectCheckBox<IonType>(
			"Y_NH3IONS", "Y-NH3 Ions", 3, IonType.Y_NH3);

	private JRcpaObjectCheckBox<IonType> y_H2OIons = new JRcpaObjectCheckBox<IonType>(
			"Y_H2OIONS", "Y-H2O Ions", 4, IonType.Y_H2O);

	private JRcpaObjectCheckBox<IonType> zIons = new JRcpaObjectCheckBox<IonType>(
			"Z_IONS", "Z Ions", 6, IonType.Z);
	...
	private JRcpaTextField txtSequence = new JRcpaTextField("Sequence",
			"Peptide Sequence", "", true);

	private JRcpaTextField txtPeakTolerance = new JRcpaTextField("PeakTolerance",
			"Peak Tolerance (da)", "0.3", true);

	private JRcpaModificationTextField txtStaticModification = new JRcpaModificationTextField(
			"StaticModification", "Static Modification (as: C +57.02 STY -17.99)", "",
			false);

	private JRcpaModificationTextField txtDynamicModification = new JRcpaModificationTextField(
			"DynamicModification", "Dynamic Modification (as: @ +79.99 # +15.99)",
			"", false);

	private JRcpaFileField txtDtaFile = new JRcpaFileField("DtaFile",
			new OpenFileArgument("Dta", "dta"), true);

	private JRcpaFileField txtResultFile = new JRcpaFileField("ResultFile",
			new SaveFileArgument("Result Picture", "jpg"), false);
	
	private NavigableImagePanel pnlImage = new NavigableImagePanel();

	private JRcpaComponentProxy pnlImageProxy = new JRcpaComponentProxy(pnlImage, 1.0,
	...
	protected void doRealGo() {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Input Peak Tolerance (da) First",
	...
	private List<IonType> getIonTypesFromComponentList(
			JRcpaHorizontalComponentList lst) {
		List<IonType> result = new ArrayList<IonType>();
		for (IRcpaComponent comp : lst.getComponents()) {
			if (comp instanceof JRcpaObjectCheckBox) {
				JRcpaObjectCheckBox ocb = (JRcpaObjectCheckBox) comp;
				if (ocb.isSelected()) {
					result.add((IonType) ocb.getObject());
				}
			}
		}
		return result;
